...
<div class="text-center mb-1">
    ...
    <h5 class="d-inline">{{date('Y', $current_ym_first)}}</h5>
    <h3 class="d-inline">{{date('M', $current_ym_first)}}</h3>
    ...
    <div class="d-inline align-bottom ml-3">
        <a href="{{url('/')}}/plugin/calendars/index/{{$page->id}}/{{$frame_id}}?year{{$frame_id}}={{date('Y')}}&month{{$frame_id}}={{date('m')}}#frame-{{$frame_id}}" class="badge badge-pill badge-info">To this month</a>
    </div>
</div>
...
<table class="table table-bordered">
    ...
    <tr class="thead d-none d-md-table-row">
        <th class="cc-w13pct text-center cc-color-sunday">Sun.</th>
        @foreach (['Mon.', 'Tue.', 'Wed.', 'Thu.', 'Fri.'] as $dayOfWeek)
        <th class="cc-w13pct text-center">{{ $dayOfWeek }}</th>
        @endforeach
        <th class="cc-w13pct text-center cc-color-saturday">Sat.</th>
    </tr>
    </thead>
    <tbody>
